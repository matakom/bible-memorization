...
\begin{document}
	...
	V rámci práce na maturitním projektu se podařilo úspěšně naimplementovat plně funkční klientskou aplikaci i serverovou část. Aplikace obsahuje všechny funkce specifikované v zadání. Uživatelé se mohou registrovat a přihlašovat pomocí účtu Google, procházet biblické texty a ukládat si verše k memorizaci. Úspěšně byl integrován algoritmus intervalového opakování (SM-2), který na základě úspěšnosti v minihrách dynamicky plánuje další revize textu. Funkční je rovněž offline režim, který ukládá postup do lokální databáze SQLite a po připojení k internetu data bezpečně synchronizuje se vzdáleným serverem. Sociální modul umožňuje propojování uživatelů přes unikátní kódy a porovnávání aktivity v žebříčcích.
	...
	Ve srovnání s aplikacemi analyzovanými v úvodu práce přináší Bible Memorizer několik zásadních inovací. Na rozdíl od většiny konkurence nevyžaduje manuální zadávání referencí, ale nabízí integrovanou čtečku textu. Zásadní výhodou je implementace intervalového opakování, které u mnoha alternativ chybí. Sociální funkce a žebříčky jsou navíc spravedlivější, protože hodnotí skutečnou aktivitu v minihrách a čas strávený učením, nikoliv jen subjektivní odškrtnutí naučeného verše. Aplikace také plně podporuje českou lokalizaci.
