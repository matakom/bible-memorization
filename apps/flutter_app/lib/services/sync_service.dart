...
class SyncService {
  ...
  SyncService(this._dio, this._db, this._prefs, this.ref);
  ...
  Future<void> _pushChanges() async {
    ...
    if (dirtyVerses.isNotEmpty) {
      await (_db.update(_db.savedVerses)
            ..where((t) => t.id.isIn(dirtyVerses.map((e) => e.id).toList())))
          .write(const db.SavedVersesCompanion(needsSync: Value(false)));
    }
    if (dirtyExercises.isNotEmpty) {
      await (_db.update(_db.exercises)
            ..where((t) => t.id.isIn(dirtyExercises.map((e) => e.id).toList())))
    ...
    if (dirtyFriendships.isNotEmpty) {
      await (_db.update(_db.friendships)..where(
            (t) => t.id.isIn(dirtyFriendships.map((e) => e.id).toList()),
          ))
  ...
  Future<void> _pullChanges() async {
    ...
    if (changes['user'] != null && (changes['user'] as List).isNotEmpty) {
      final serverUser = changes['user'][0];
      final String serverUserId = serverUser['id'].toString();
      final String? localUserId = _prefs.getString(_kUserIdKey);
      ...
      if (localUserId == null) {
        await _prefs.setString(_kUserIdKey, serverUserId);
      }
    }
    ...
    if (data['timestamp'] != null)
      await _prefs.setString(_kLastSyncKey, data['timestamp']);
  }
  ...
  db.ExercisesCompanion _exerciseFromJson(Map<String, dynamic> json) =>
      db.ExercisesCompanion.insert(
        ...
        grade: json['grade'],
        exerciseType: GameType.values.firstWhere(
  ...
  db.UsersCompanion _userFromJson(Map<String, dynamic> json) {
    try {
      return db.UsersCompanion.insert(
        ...
        friendCode: Value(json['friendCode'] ?? ''),
...
final syncServiceProvider = FutureProvider<SyncService>((ref) async {
  ...
  return SyncService(dio, database, prefs, ref);
});
